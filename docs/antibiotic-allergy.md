...
| アレルギー歴 | 代替候補 | 交差反応・忍容性の目安 | 実臨床での考え方 |
|---|---|---:|---|
| ペニシリン系 | 同一R1側鎖のアミノセファロスポリン | **16.45%**（95% CI 11.07〜23.75） | 選択的ABPC/AMPCアレルギーが確認されていれば避ける |
| ペニシリン系 | R1側鎖の類似度が低いセフェム | **2.11%**（95% CI 0.98〜4.46） | セファゾリン、CTRX、CAZ、CFPMなどは低リスク |
| ペニシリン系 | カルバペネム | **0.87%**（95% CI 0.32〜2.32） | SCARを除き、通常は事前検査なしで投与可能 |
| ペニシリン／セフェム系 | AZT（アズトレオナム） | **極めて低い／実証されていない** | CAZとは同一R1側鎖のため、CAZ確定アレルギーでは注意 |
| キノロン系 | 別のキノロン | **2.5%**（95% CI 1.2〜4.9） | 即時反応歴321曝露の後ろ向き研究。全剤一律回避は必須ではない |
| マクロライド系 | 別のマクロライド | **不明** | 交差反応は報告されるが、小規模研究のみ。薬物チャレンジで確認 |
| テトラサイクリン系 | 別のテトラサイクリン | **不明** | 主に固定薬疹の症例報告・小規模研究。反応型ごとに判断 |
| ST合剤などサルファ抗菌薬 | 非抗菌薬サルホンアミド | **免疫学的交差反応なし** | フロセミド、チアジド、SU薬、セレコキシブなどを一律回避しない |
| アミノグリコシド系 | 別のデオキシストレプタミン系薬 | **約50%以上** | 主に接触アレルギーのデータ。GM、TOB、AMK、KM、FRM、NEO間は注意 |
| VCMによるDRESS | TEIC／テラバンシン | **2/15、3/11でex vivo交差反応** | 少数・検査上のデータ。専門医評価なしの安易な置換を避ける |
| VCMによるDRESS | ダルババンシン | **0/15でex vivo交差反応なし** | 臨床投与試験ではなく、半減期も長いため慎重に判断 |
| CLDM／LZD | 他系統薬 | **確立した交差反応データなし** | 被疑薬ごとに評価する |

!!! warning "表の読み方"
    数値は研究対象、反応型、診断確度が異なるため、患者個人の発症確率を直接示すものではない。「薬剤名だけの未確認ラベル」と、薬物チャレンジ・皮膚試験などで確認されたアレルギーは分けて考える。SCARや臓器障害型反応には、この表をそのまま適用しない。
...
- ABPC/SBT（アンピシリン・スルバクタム）アレルギー歴があっても、**R1側鎖が異なるセフェムの交差反応性は低い**
- 交差反応はβラクタム環そのものより、主に**R1側鎖の構造類似性**で決まる
- 従来の8〜10%という推定は、1980年以前の製剤汚染や対象薬の偏りによる過大評価と考えられる
- **セファゾリンは独自の側鎖**を持ち、ペニシリン系との交差反応性は極めて低い
...
!!! note "ABPCとAMPCの側鎖"
    ABPCと同一のR1側鎖を持つ代表薬はセファレキシン、セファクロル。AMPCではセファドロキシル、セフプロジルなどが該当する。選択的ABPC/AMPCアレルギーが証明されている場合は、対応する同一R1側鎖薬を避ける。

AAAAI 2022年薬物アレルギー診療パラメータでは、以下が条件付きで推奨されている。
